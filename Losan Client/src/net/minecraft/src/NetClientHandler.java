...
import Losan.Gui.GuiBienvenue;
import Losan.Packet.Packet133Money;
...
public class NetClientHandler extends NetHandler
{
    ...
    public void handleVehicleSpawn(Packet23VehicleSpawn par1Packet23VehicleSpawn)
    {
        ...
        else if (par1Packet23VehicleSpawn.type == 110)
        {
            var8 = new EntityVoiture(this.worldClient, var2, var4, var6);
        }
    ...
    public void handleOpenWindow(Packet100OpenWindow par1Packet100OpenWindow)
    {
        ...
        switch (par1Packet100OpenWindow.inventoryType)
        {
            ...
            case 9://ajout
                var2.displayGUIFreezer1(new TileEntityFreezer());
                var2.openContainer.windowId = par1Packet100OpenWindow.windowId;
                break;
            
            case 10://ajout
                var2.displayGUIFut(new TileEntityFut());
                var2.openContainer.windowId = par1Packet100OpenWindow.windowId;
                break;
            
            case 11://ajout
            	var2.displayGUIPoubelle(new InventoryBasic(par1Packet100OpenWindow.windowTitle, par1Packet100OpenWindow.slotsCount));
                var2.openContainer.windowId = par1Packet100OpenWindow.windowId;
                break;
        }
    }
    ...
    public void handleMoney(Packet133Money packet){//ajout
        if(mc.thePlayer.entityId == packet.playerId){
                mc.thePlayer.money = packet.money;                
        }
    }
    
    public void handleDroit(Packet135Droit packet){//ajout
    	
    	if(mc.thePlayer.username.equals(packet.name)){
    		if(packet.cond == true){ 	
    			this.mc.displayGuiScreen(new GuiBienvenue(this.mc));
    		}
    	}
    }
}
