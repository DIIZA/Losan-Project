...
import Losan.Packet.Packet135Droit;
import Losan.Utils.DescriptionFinder;

public class GuiIngame extends Gui
{
    private double hauteurTailleD = 512D;//ajout
    private float hauteurTailleF = 512F;//ajout
    private int hexTaille = 0x1f0; //0xf0 pour 256 //ajout
	private int compteur20 = 20;
	private boolean cond20 = false;
	
    //debut ajout
	public boolean pointage = false;	
	public int decalage = 153;
	public int id;
	public int data;
	public String name;
	...
	public int[] list = {26, 34, 55, 93, 117, 118, 140, 144};
	public boolean screenDesc = false;
	public boolean screenblock = true;
    //fin ajout
...
//  	  System.out.println((6.2831855*60)/360);
        
        //debut ajout
        int x = var6;
        int y = var7;
        
        //on ouvre une nouvelle matrice
        GL11.glPushMatrix();
        
        //affichage général du gui
        DescriptionFinder.drawCadre(x, y, this);

        //si l'item existe et à un nom
        if(this.item != null && !this.name.equals("")){
        	//on affiche l'item
        	itemRenderer.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, this.item, (int)((x-139))+this.decalage, 42);
        	
        	//fix pour ne pas faire bugger la lumière des autres éléments dans guiIngame (ex la barre d'inventaire)
        	RenderHelper.disableStandardItemLighting();
        	GL11.glDisable(GL12.GL_RESCALE_NORMAL);
        	
        //si c'est une entity à afficher
        }else if(this.mob != null && this.pointage)
        	DescriptionFinder.displayMob(x, y, this);
        
        //on ferme notre matrice
        GL11.glPopMatrix();
        //fin ajout
    ...
    private void renderPortalOverlay(float par1, int par2, int par3)
    {
        ...
        float var5 = (float)(Block.portal.blockIndexInTexture / 16) / (16.0F * (hauteurTailleF / 256F));
        float var6 = (float)(Block.portal.blockIndexInTexture % 16 + 1) / 16.0F;
        float var7 = (float)(Block.portal.blockIndexInTexture / 16 + 1) / (16.0F * (hauteurTailleF / 256F));
    ...
    public void updateTick()
    {
    	...
    	{
    		DescriptionFinder.checkGuiDesc(this);
    		
    		if(this.screenDesc && (Keyboard.isKeyDown(Keyboard.KEY_ESCAPE) || Keyboard.isKeyDown(Keyboard.KEY_E))){
    			this.screenDesc = false;
    			this.screenblock = false;
    			this.mc.thePlayer.closeScreen();
    		}
    		
    		if(Keyboard.isKeyDown(Keyboard.KEY_I) && this.screenblock && DescriptionFinder.fixChat){
    			if(this.screenDesc)
    				DescriptionFinder.decompte = 80;
    			this.screenDesc = !this.screenDesc;
    			this.screenblock = false;
    			
    		}
    		
    		if(!DescriptionFinder.fixChat){
    			this.screenDesc = false;
    			this.screenblock = false;
    		}
    		
    		if(!this.pointage && this.screenDesc)
    			DescriptionFinder.decompte(this);

    		if(this.mc.thePlayer.scs){
    			if(Mouse.isButtonDown(1) || Mouse.isButtonDown(0)){    			
    				this.cond20 = true;
    				this.mc.thePlayer.scs = false;
    				this.compteur20 = 20;
    			}
    		}
    		...
    		if(this.cond20){		
    			--this.compteur20;
    			if(this.compteur20 < 0){	
    				this.cond20 = false;
    				mc.thePlayer.sendChatMessage("/scs reload");
    			}
    		}

    		if(!this.mc.thePlayer.init )
    		{
    			this.mc.getSendQueue().addToSendQueue(new Packet135Droit(this.mc.thePlayer));
    			this.mc.thePlayer.init = true;
    		}
    	}
